...
func _on_battle_started(player: BattleUnit, monster: BattleUnit) -> void:
	...
	match monster_type:
		...
		"boss":
			...
			_monster_avatar.get_node("Sprite").custom_minimum_size = Vector2(160, 200)
			_monster_card.get_node("StatusText").text = "‼ 地牢领主"
			badge.add_theme_font_size_override("font_size", 18)
	_update_ui()

func _set_monster_visual(glow: Color, body: Color, icon: String, badge_text: String, badge_color: Color) -> void:
	_monster_avatar.get_node("Glow").color = glow
	_monster_avatar.get_node("Sprite").color = body
	_monster_avatar.get_node("Icon").text = icon
	var badge := $TopBar/TopHBox/MonsterBadge
...
func _on_battle_ended(player_won: bool, rewards: Dictionary) -> void:
	...
	if player_won:
		...
		if monster_type == "boss":
			_on_log_message("[color=#ffaa44]🎉 恭喜通关！你击败了地牢领主！[/color]")
			Game.mark_dungeon_cleared(Game.current_dungeon_id)
			SaveManager.save_game()
			await get_tree().create_timer(3.0).timeout
			var map_scene: Node = load("res://scenes/map/MapSelectScene.tscn").instantiate() as Node
			map_scene.set_meta("show_victory", true)
...
func _return_to_dungeon() -> void:
	...
	PlayerData.current_hp = mini(PlayerData.current_hp + int(stats["max_hp"] * 0.5), stats["max_hp"])
	PlayerData.current_mp = mini(PlayerData.current_mp + int(stats["max_mp"] * 0.5), stats["max_mp"])
	SaveManager.save_game()
...
func _update_skill_buttons() -> void:
	...
	var skills_data = [
		...
		{"id": "blood_slash", "btn": $SkillPanel/SkillBar/Skill3, "name": "🩸 斩击", "key": "3"},
	]
